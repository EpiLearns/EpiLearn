...
static void block_class_init(BlockClass*klass)
{
    ...
    g_object_class_install_property(gclass,PROP_SIZ,g_param_spec_int("size","Size","size of block",1,100,10,G_PARAM_READWRITE));


    g_object_class_install_property(gclass,PROP_COLOR,g_param_spec_string("color","Color-Desc","Quark of color descripter for parse",COLOR_CLEAR,G_PARAM_READWRITE));
}
...
TetChecker *tet_checker_new(int height,int width,int block_siz)
{
    ...
    for (i = 0; i < height; i++) 
    {
        ...
        for (j = 0; j < width; j++) 
        {
            ...
            gtk_grid_attach(GTK_GRID(checker->container),checker->grid[i*width+j], j, i, 1, 1);
        }
...
int tet_checker_eliminate(TetChecker*checker,int baseline)
{
    ...
    for (i = baseline; i >=0; ) 
    {
        ...
        if(flag)
        {
            //need to eliminat current line,and move all lines beyond to down
